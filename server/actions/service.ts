...
export async function serviceAction(data: z.infer<typeof ServiceSchema>) {
  try {
    ...
    // Drizzle returns inserted id via "returning"
    const createdId = newServiceInquiry?.insertId || uuidv4()
...
export async function createNewQuotation(data: z.infer<typeof CreateQuotationSchema>) {
  try {
    // Validate input data
    ...
    throw error instanceof Error ? error : new Error("Failed to create quotation")
  }
}
...
export async function deleteQuotation(quotationId: string) {
   try {
    ...
    if (error instanceof Error && error.message.includes("foreign key constraint")) {
      throw new Error("Cannot delete quotation because it's referenced by other records")
    }
    
    throw error instanceof Error ? error : new Error("Failed to delete quotation")
  }
}
...
export async function addOrEditQuotationItem(
  ...
  try {
    ...
    const quantity = typeof validatedData.quantity === 'string' 
      ? parseFloat(validatedData.quantity) 
      : validatedData.quantity
    
    const unitPrice = typeof validatedData.unitPrice === 'string' 
    ...
    await db.insert(auditLogs).values({
      ...
      userId: session.user.id,
      details: JSON.stringify({
        action: actionType === "CREATE" ? "Quotation item added" : "Quotation item updated",
        data: {
          id: recordId,
          serviceInquiryId,
          description: validatedData.description,
    ...
    return { success: actionType === "CREATE" ? "Quotation item added successfully" : "Quotation item updated successfully" }
  } catch (error) {
    console.error("Error in addOrEditQuotationItem:", error)
...
export async function updateQuotationWithItemsAndTax(data: z.infer<typeof UpdateQuotationWithTaxSchema>) {
  try {
    // Validate input data
    ...
    revalidatePath("/admin/service-inquiries")
    revalidatePath(`/admin/service-inquiries/${validatedData.serviceInquiryId}`)
    ...
    console.error("Error in updateQuotationWithItemsAndTax:", error)
...
export async function deleteQuotationItem(itemId: string, serviceInquiryId: string) {
  try {
    const headersList = await headers()
    const ipAddress = headersList.get("x-forwarded-for") || "unknown"
    const userAgent = headersList.get("user-agent") || "unknown"
...
export async function updateQuotationDetails(data: z.infer<typeof QuotationDetailsSchema>) {
  try {
    // Validate input data
    ...
    console.error("Error in updateQuotationDetails:", error)
...
async function recalculateQuotationTotals(serviceInquiryId: string, newTaxRate?: number) {
  try {
    // Get all quotation items for this service inquiry using SQL SUM
    const [sumResult] = await db
      .select({
        subtotal: sql<number>`COALESCE(SUM(CAST(${quotationItems.total} AS DECIMAL(10, 2))), 0)`.as('subtotal')
      })
      .from(quotationItems)
      .where(eq(quotationItems.quotationId, serviceInquiryId))

    // Ensure subtotal is a number
    const subtotal = typeof sumResult?.subtotal === 'string' 
...
export async function getServiceInquiryById(id: string) {
  try {
    ...
    console.error("Error in getServiceInquiryById:", error)
    throw new Error("Failed to fetch service inquiry")
  }
}

// Update read status of service inquiry
export async function updateServiceInquiryReadStatus(id: string, read: boolean) {
  try {
    const headersList = await headers()
    const ipAddress = headersList.get("x-forwarded-for") || "unknown"
    const userAgent = headersList.get("user-agent") || "unknown"
    ...
    await db.insert(auditLogs).values({
      ...
      userId: session.user.id,
      details: JSON.stringify({
        action: read ? "Service inquiry marked as read" : "Service inquiry marked as unread",
        data: { id, read },
      }),
      ipAddress,
      userAgent,
    ...
    return { success: `Service inquiry marked as ${read ? 'read' : 'unread'} successfully` }
  } catch (error) {
    console.error("Error in updateServiceInquiryReadStatus:", error)
    return { error: "An unexpected error occurred. Please try again later." }
  }
}
...
export async function getQuotationTotals(serviceInquiryId: string) {
  try {
    ...
    const subtotal = inquiry.subtotal ? parseFloat(inquiry.subtotal.toString()) : 0
    const taxRate = inquiry.taxRate ? parseFloat(inquiry.taxRate.toString()) : 0
    ...
    console.error("Error in getQuotationTotals:", error)
